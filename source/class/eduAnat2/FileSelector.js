...
qx.Class.define("eduAnat2.FileSelector", {
	...
	members : {
		...
		getFile : async function ( func ) {
			...
			this.set( { width : minSize, height : minSize } );
			...
			fileBrowser.setFileHandler( file => {
				...
				this.fireDataEvent( "file", { file } );

			} );

			const closeHandler = this.addListenerOnce( "close", () => {

				this.fireDataEvent( "file", { canceled : true });

			} );

			const okButtonHandler = okButton.addListenerOnce( "execute", () => {

				this.fireDataEvent( "file", { file : fileBrowser.getSelectedFiles()[ 0 ] } );

			} );

			const data = await ( new Promise( res =>
				this.addListenerOnce( "file", d => { res( d.getData() ) } ) ) );
